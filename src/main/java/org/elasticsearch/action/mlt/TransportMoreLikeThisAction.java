...
public class TransportMoreLikeThisAction extends HandledTransportAction<MoreLikeThisRequest, SearchResponse> {
    ...
    protected void doExecute(final MoreLikeThisRequest request, final ActionListener<SearchResponse> listener) {
        ...
        // redirects to data node that has the index locally available
        ...
        MoreLikeThisQueryBuilder mlt = makeMoreLikeThis(request);
        ...
        SearchSourceBuilder extraSource = searchSource().query(mlt);
    ...
    private MoreLikeThisQueryBuilder makeMoreLikeThis(MoreLikeThisRequest request) {
        Item item = (Item) new Item(request.index(), request.type(), request.id())
